<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" href="css/header.css">
	<link rel="stylesheet" href="css/product.css">
	<link rel="stylesheet" href="css/cart.css">
	<link rel="stylesheet" href="css/footer.css">
	<link rel="stylesheet" href="css/home.css">
	<link rel="icon" href="icons/logoMS.svg" type="image/svg">
	<title>Ecommerce</title>
</head>
<body>
	<nav class="topBar flex spcbtw navbar bgWhite">
		<div class="burgerItem">
			<img src="icons/burgerIcon.png" id="openBurger">
			<img src="icons/x.png" id="closeBurger">
		</div>
		<div class="titleItem pdRight">
			<a class="titleSite" href="home">Ecommerce</a>
		</div>
		<div class="searchItem" id="search">
			<img src="icons/search.png">
		</div>
		<div class="shopItem">
			<img src="icons/shoppingCart.png" id="openCart">
		</div>
	</nav>
	<div class="searchBar bgWhite">
		<div class="flex spcbtw">
			<div class="burgerItem">
				<img src="icons/search.png">
			</div>
			<div class="titleItem">
				<input type="text" placeholder="Búsqueda" class="searchInput">
			</div>
			<div class="shopItem">
				<img src="icons/x.png" id="closeSearch">
			</div>
		</div>
	</div>
	<div class=sideBar>
		<div class="list">
			<ul>
				<div><li>Novedades</li></div>
				<div><li>Mujer</li></div>
				<div><li>Hombre</li></div>
				<div><li>Unisex</li></div>
			<ul>
		</div>
		<div class="faq">
			<ul>
				<li>Ayuda / Preguntas frecuentes</li>
				<li>Iniciar Sesión</li>
				<li>Acerca de</li>
			<ul>
		</div>
	</div>
	<div class="cart">
		<div class="topBar flex spcbtw greyShadow">
			<span class="titleSite burgerItem">Carrito</span>
			<div class="shopItem">
				<img src="icons/x.png" id="closeCart">
			</div>
		</div>
		<div class="pdLeft pdTop3 pdBot3">
			<div class="flex">
				<div class="bgPink imgItemCart mgRight">
					<img src="images/image7.png">
				</div>
				<div class="contentItemCart pdRight">
					<div class=" subItemCart pdBot3">
						<div class="flex spcbtw">
							<div class="bold font20">Sweater Nelson</div>
							<div class=""><img src="icons/trash.png"></div>
						</div>
					</div>
					<div class=" subItemCart pdBot3">
						<div class="flex">
							<div class="pdRight font18"><span class="bold">Color:</span> Naranja</div>
							<div class="font18"><span class="bold">Talle:</span> XL</div>
						</div>
					</div>
					<div class=" subItemCart pdBot3">
						<div class="flex spcbtw">
							<div class="">
								<div class="flex">
									<img src="icons/minus.png" class="" name="">
									<input type="number" class="inputCart" min="1" max="10" value="1">
									<img src="icons/plus.png" class="" name="">
								</div>
							</div>
							<div class="font20">$999</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="pdLeft pdRight">
			<div class="shadow"></div>
		</div>
		<div class="pdLeft pdTop3 pdBot3">
			<div class="flex">
				<div class="bgPink imgItemCart mgRight">
					<img src="images/image7.png">
				</div>
				<div class="contentItemCart pdRight">
					<div class=" subItemCart pdBot3">
						<div class="flex spcbtw">
							<div class="bold font20">Sweater Nelson</div>
							<div class=""><img src="icons/trash.png"></div>
						</div>
					</div>
					<div class=" subItemCart pdBot3">
						<div class="flex">
							<div class="pdRight font18"><span class="bold">Color:</span> Naranja</div>
							<div class="font18"><span class="bold">Talle:</span> XL</div>
						</div>
					</div>
					<div class=" subItemCart pdBot3">
						<div class="flex spcbtw">
							<div class="">
								<div class="flex">
									<img src="icons/minus.png" class="" name="">
									<input type="number" class="inputCart" min="1" max="10" value="1">
									<img src="icons/plus.png" class="" name="">
								</div>
							</div>
							<div class="font20">$999</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="pdLeft pdRight">
			<div class="shadow"></div>
		</div>
	</div>

